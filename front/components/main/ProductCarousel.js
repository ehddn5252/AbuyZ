...
export default function ProductCarousel() {
  ...
  useEffect(() => {
    setArray([
      { name: "당근", price: 690, grade: 4.0, reviews: 100 },
  ...
  return (
    <Container style={{ background: "#ffffff" }}>
      <Title>{username}님을 위한 추천상품</Title>
      <StyledSlider {...settings}>
        {array.map((e, idx) => (
          <CardContainer key={idx}>
            <CardImg alt="추천상품" src="/images/carrot.png" />
            <CardName>{e.name}</CardName>
            <CardTextBox>
              <CardPrice>{e.price.toLocaleString("ko-KR")}</CardPrice>
              <Won>원</Won>
            </CardTextBox>
            <CardTextBox>
              <Star alt="별점" src="/images/star.png" />
              <CardGrade>{e.grade}</CardGrade>
